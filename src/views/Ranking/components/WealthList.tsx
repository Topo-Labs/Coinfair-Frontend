...
export default function WealthList({ timeType, type, pageName }) {
  ...
  const addressesList = list?.map((item: { wallet: string; }) => item.wallet)
  const addressesNameList = useENSName(addressesList)
  const nonEmptyNameObjects = addressesNameList.filter((item: { name: any; }) => item.name);
  console.info("fetch name service in wealthList:\n", addressesNameList)

  const updatedList = list?.map((listItem: { wallet: string; }) => {
    const matchedObject = nonEmptyNameObjects.find(nonEmptyItem =>
      nonEmptyItem.account.toLowerCase() === listItem.wallet.toLowerCase()
    );
    if (matchedObject) {
      return { ...listItem, shortWallet: matchedObject.name }
    }
    return listItem;
  }
  )
