...
const config = {
  ...
  '56': {
    pre_url: 'https://api.bscscan.com/api?',
    api_key: 'DMXYVI3I31TQJ1YYVUA7XBMRJIPKK8FUK8',
    factory_address: '',
...
async function creatorAddress(req: NextApiRequest): Promise<{ creatorAddress: string; alreadyExist: number }> {
  ...
  const requestUrl = `${config[argChainId].pre_url}?module=contract&action=getcontractcreation&contractaddresses=${token}&apikey=${config[argChainId].api_key}`
